...
const opponents = huskyGames2016.map(game => extractOpponent(game));
...
const gamesLost = huskyGames2016.filter(game => huskiesLost(game));
console.log(gamesLost);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!

const opponentsLost = gamesLost.map(game => extractOpponent(game));
console.log(opponentsLost);
...
let fumbleGames = huskyGames2016.filter(game => game.fumbles >= 1);
...
function mostYardsPassing(g1, g2) {
  if (g1.passing_yards == undefined) {
    return g2;
  }
  if (g1.passing_yards > g2.passing_yards) {
    return g1;
  }
  return g2;
}
...
const mostPassingGame = huskyGames2016.reduce(mostYardsPassing, { passingYards: 0 });
...
function fumble(game) {
  return game.fumbles > 0;
}

const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, fumble);
...
const arr = huskyGames2016.filter(fumbledAndLostFilter);
console.log(arr);
