...
dobInput.addEventListener("input", function() {
  ...
  const age = getYearsSince(date);

  if (age < 13 || age > 200) {
    const errorMessage = "You need to be at least 13 years old.";
    dobInput.setCustomValidity(errorMessage);
    dobFeedback.textContent = errorMessage;
  }
  else {
...
function validatePasswordMatch() {
  ...
  const confirmfeedback = document.getElementById("passwordConfirmFeedback");

  if (password.value !== pwconfirm.value){
    const errormessage = "Passwords do not match";
    pwconfirm.setCustomValidity(errormessage);
    confirmfeedback.textContent = errormessage;
  }
  else {
...
const passwordInputs = document.querySelectorAll("#passwordInput, #passwordConfirmInput");
...
const inputs = document.querySelectorAll("#emailInput, #passwordInput, #passwordConfirmInput, #dobInput");
